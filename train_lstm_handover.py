"""
Trains an LSTM to IMITATE the vertical handover policy found in the
dataset generated by vho_sim.py (log_decisions=True) / optimize_vho.py, with the
goal of using this network as an additional handover decision scheme
("LSTM-VHO") within run_experiments.py, for comparison with I-VHO, D-VHO,
and LA-VHO.

IMPORTANT DIFFERENCE compared to the original script (wind speed
prediction): here, the problem is one of BINARY CLASSIFICATION, not regression.
Given a recent history of the link state (geometric coverage, shadowing
blockage, link availability, speed, position, and the previous mode),
the network predicts whether the user should be connected via VLC (1) or
via WLAN/RF (0) at the current step. Therefore, the loss function is
BCEWithLogitsLoss (not MSE) and the evaluation metrics are classification-based
(accuracy, precision, recall, F1, ROC-AUC, confusion matrix) rather than
RMSE/MAE/R2—though I still report MAE/RMSE between the predicted
probability and the label (0/1) as an additional measure of "error,"
since you requested this calculation.
"""
